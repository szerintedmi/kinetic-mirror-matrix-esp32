...
private:
  ...
  CommandResult handleMove(const std::string &args, CommandExecutionContext &context, uint32_t now_ms);
  CommandResult handleHome(const std::string &args, CommandExecutionContext &context, uint32_t now_ms);
};
