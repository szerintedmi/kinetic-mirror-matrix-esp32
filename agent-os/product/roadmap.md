...
6. [ ] 16+ Motors per ESP32 (Shared STEP Spike) — Adopt shared STEP with per‑motor SLEEP gating via existing 74HC595 (no new gating hardware). Implement hardware‑timed STEP generator (ESP32 RMT/LEDC) targeting 10 ksteps/s; replace/override FAS as needed. Unify to global SPEED/ACCEL (no per‑move overrides; reject changes while moving). Use µs‑scale DIR guards aligned to STEP gaps. Validate via manual bench (physical vs MCU position). Deliver firmware gating/timing notes. Depends on: 2, 4. `S`
