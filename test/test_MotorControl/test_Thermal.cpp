...
void test_get_thermal_runtime_limiting_default_on_and_max_budget() {
  ...
  TEST_ASSERT_TRUE(r1.find("CTRL:ACK THERMAL_LIMITING=ON") != std::string::npos);
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", s.c_str());
...
void test_preflight_e10_move_enabled_err() {
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET SPEED=1", 0).c_str());
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET ACCEL=1000", 0).c_str());
...
void test_preflight_e11_move_enabled_err() {
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("WAKE:0", 0).c_str());
  // After 100s, budget will be <= 0
  (void)p.processLine("STATUS", 100000);
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET SPEED=4000", 0).c_str());
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET ACCEL=16000", 0).c_str());
...
void test_preflight_warn_when_disabled_then_ok() {
  MotorCommandProcessor p;
  // Disable limits
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET THERMAL_LIMITING=OFF", 0).c_str());
  ...
  bool r1_ok = (r1.find("\nCTRL:ACK est_ms=") != std::string::npos);
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("WAKE:1", 0).c_str());
  (void)p.processLine("STATUS", 100000);
...
void test_move_ok_returns_estimate() {
  ...
  TEST_ASSERT_TRUE(r.find("CTRL:ACK est_ms=") == 0);
}
...
void test_last_op_timing_move() {
  MotorCommandProcessor p;
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET SPEED=1000", 0).c_str());
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET ACCEL=1000", 0).c_str());
...
void test_wake_reject_enabled_no_budget() {
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SLEEP:0", 100000).c_str());
...
void test_auto_sleep_overrun_cancels_move_and_awake() {
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET ACCEL=1000", 0).c_str());
  std::string r = p.processLine("MOVE:0,1200", 0);
  TEST_ASSERT_TRUE(r.find("CTRL:ACK est_ms=") != std::string::npos);
  // Re-enable enforcement
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", p.processLine("SET THERMAL_LIMITING=ON", 0).c_str());
