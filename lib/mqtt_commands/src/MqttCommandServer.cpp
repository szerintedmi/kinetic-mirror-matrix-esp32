...
namespace mqtt
{
        ...
        config_(cfg)
  {
    ...
    dispatcher_token_ = transport::response::ResponseDispatcher::Instance().RegisterSink(
        [this](const transport::response::Event &evt)
        { this->handleDispatcherEvent(evt); });
  }
  ...
  bool MqttCommandServer::handleDuplicateCommand(const std::string &cmd_id, uint32_t now_ms)
  {
    ...
    if (transport::response::ResponseDispatcher::Instance().Replay(
            cmd_id, [this](const transport::response::Event &evt)
            { this->handleDispatcherEvent(evt); }))
    {
      return true;
    }
  ...
  void MqttCommandServer::respondWithHelp(const std::string &cmd_id, uint32_t now_ms)
  {
    ...
    if (log_)
    {
      ...
      while (start <= help_text.size())
      {
        ...
        std::string line = help_text.substr(start, end == std::string::npos ? std::string::npos : end - start);
        log_(line);
#if defined(ARDUINO) && defined(ESP32)
        delay(0);
#endif
