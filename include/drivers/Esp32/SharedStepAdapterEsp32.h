...
public:
  ...
  void setAutoEnable([[maybe_unused]] uint8_t motor_id, [[maybe_unused]] bool auto_enable) override {}
...
private:
  ...
  struct DivisionOperands {
    ...
    explicit constexpr DivisionOperands(uint64_t num = 0, uint64_t den = 0) : numerator(num), denominator(den) {}
  };
  static inline uint32_t ceil_div_u32(DivisionOperands operands)
  {
    ...
    return static_cast<uint32_t>((operands.numerator + operands.denominator - 1ULL) / operands.denominator);
  }

  // Edge hook plumbing (ISR context)
  static void IRAM_ATTR onRisingEdgeHook_();
  static SharedStepAdapterEsp32 *self_; // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
