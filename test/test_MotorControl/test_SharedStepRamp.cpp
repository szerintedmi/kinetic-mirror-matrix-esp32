...
void test_stop_distance_basic() {
  ...
  uint32_t d = stop_distance_steps(4000, 16000);
...
void test_stop_distance_edges() {
  ...
  TEST_ASSERT_EQUAL_UINT32(0, stop_distance_steps(0, 16000));
  TEST_ASSERT_EQUAL_UINT32(0, stop_distance_steps(0, 1));
  // Very small acceleration yields larger stopping distances; check rounding up
  uint32_t d = stop_distance_steps(1000, 1); // 1000^2 / 2 = 500000
