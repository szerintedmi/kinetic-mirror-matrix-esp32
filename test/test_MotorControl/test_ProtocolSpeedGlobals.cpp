...
void test_get_set_speed_ok() {
  ...
  TEST_ASSERT_TRUE(r1.rfind("CTRL:ACK SPEED=", 0) == 0);
...
void test_get_set_accel_ok() {
  ...
  TEST_ASSERT_TRUE(r1.rfind("CTRL:ACK ACCEL=", 0) == 0);
...
void test_get_set_decel_ok() {
  ...
  TEST_ASSERT_TRUE(r1.rfind("CTRL:ACK DECEL=", 0) == 0);
...
void test_set_decel_busy_reject() {
  MotorCommandProcessor proto;
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", proto.processLine("SET SPEED=4000", 0).c_str());
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", proto.processLine("SET ACCEL=16000", 0).c_str());
...
void test_home_uses_speed_accel_globals() {
  ...
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", proto.processLine("SET SPEED=2000", 0).c_str());
  TEST_ASSERT_EQUAL_STRING("CTRL:ACK", proto.processLine("SET ACCEL=8000", 0).c_str());
