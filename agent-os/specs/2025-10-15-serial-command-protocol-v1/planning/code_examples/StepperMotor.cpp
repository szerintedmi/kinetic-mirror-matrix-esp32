...
StepperMotor::StepperMotor(uint16_t id, const MotorPins& pins) : id_(id), pins_(pins) {}
...
void StepperMotor::beginHoming(
  ...
  if (homing_.forwardSteps == 0) {
    homing_.stage =
        homing_.backoffSteps > 0 ? HomingState::Stage::Backoff : HomingState::Stage::Center;
  }
}
