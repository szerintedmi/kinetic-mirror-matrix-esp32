...
struct SquareWaveDurations {
  uint16_t high_us = 0;
  uint16_t low_us = 0;
  explicit constexpr SquareWaveDurations(uint16_t high = 0, uint16_t low = 0)
      : high_us(high), low_us(low) {}
};
...
void IRAM_ATTR SharedStepRmtGenerator::onTxEndIsr() {
  ...
  if (half > 32767)
    half = 32767;
  rmt_item32_t item = make_square_item(
      SquareWaveDurations(static_cast<uint16_t>(half), static_cast<uint16_t>(half)));
...
void SharedStepRmtGenerator::setSpeed(uint32_t speed_sps) {
  ...
  uint32_t half = period / 2;
  if (half > 32767)
    half = 32767;  // duration field limit
