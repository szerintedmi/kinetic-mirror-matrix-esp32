...
uint32_t estimateMoveTimeMs(int64_t distance_steps, int64_t speed_sps, int64_t accel_sps2) {
  ...
  if (d <= 0)
    return 0;
  if (speed_sps <= 0)
    speed_sps = 1;  // avoid div-by-zero
  if (accel_sps2 <= 0)
    accel_sps2 = 1;  // avoid div-by-zero
...
uint32_t estimateMoveTimeMsAsym(int64_t distance_steps,
  ...
  auto ceil_div64 = [](int64_t n, int64_t d) -> int64_t {
    if (d <= 0)
      return 0;
    return (n + d - 1) / d;
  };
...
uint32_t estimateHomeTimeMsWithFullRangeSharedStep(int64_t overshoot_steps,
                                                   ...
                                                   int64_t accel_up_sps2,
                                                   int64_t decel_down_sps2) {
  uint32_t t = estimateHomeTimeMsWithFullRangeAsym(
      overshoot_steps, backoff_steps, full_range_steps, speed_sps, accel_up_sps2, decel_down_sps2);
