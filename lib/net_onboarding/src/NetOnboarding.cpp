...
static void
handleApiWifiLogic_(AsyncWebServerRequest* request, const String& ssid, const String& pass) {
  if (!g_portal_net) {
    sendJsonError_(request, 503, "net-unavailable");
    return;
  }
...
static void ip_to_str_(uint32_t ip, std::array<char, 16>& out) {
  ...
  // Arduino IPAddress::toString renders normal order; our bytes are little-endian depending on
  // platform Using WiFi.localIP() path on Arduino instead. This helper is used only for non-Arduino
  // paths.
...
int NetOnboarding::scanNetworks(std::vector<WifiScanResult>& out,
                                int max_results,
                                bool include_hidden) {
  if (!wifi_)
    wifi_ = MakeWifi();
...
void NetOnboarding::buildApSsid_(std::array<char, 32>& out) const {
  ...
  if (wifi_) {
    ...
    std::snprintf(
        out.data(), out.size(), "%s%02X%02X%02X", prefix ? prefix : "AP-", mac[3], mac[4], mac[5]);
  }
}
