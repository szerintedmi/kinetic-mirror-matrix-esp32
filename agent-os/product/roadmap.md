...
5. [x] Thermal Limits Enforcement — Enforce per‑motor runtime/cooldown budgets (derived from STATUS metrics) to cap continuous motion and require cooldown before resuming; thresholds configurable and can be disabled and/or overriden for lab use. Depends on: 4. `S`
