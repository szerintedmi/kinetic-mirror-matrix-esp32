...
void test_estimator_triangular_above_naive_bound() {
  int d = 800, v = MotorControlConstants::DEFAULT_SPEED_SPS,
      a = MotorControlConstants::DEFAULT_ACCEL_SPS2;
  uint32_t est = MotionKinematics::estimateMoveTimeMs(d, v, a);
...
void test_stub_move_uses_estimator_duration() {
  ...
  TEST_ASSERT_TRUE(
      p.processLine(std::string("SET SPEED=") + std::to_string(v), 0).rfind("CTRL:DONE", 0) == 0);
  TEST_ASSERT_TRUE(
      p.processLine(std::string("SET ACCEL=") + std::to_string(a), 0).rfind("CTRL:DONE", 0) == 0);
...
void test_stub_home_uses_estimator_duration() {
  MotorCommandProcessor p;
  uint32_t t = MotionKinematics::estimateHomeTimeMs(MotorControlConstants::DEFAULT_OVERSHOOT,
