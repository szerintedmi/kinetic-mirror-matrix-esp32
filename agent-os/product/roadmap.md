...
11. [x] Commands via MQTT — Ship a shared command schema covering all serial actions, with MQTT request/response parity (`devices/<id>/cmd` QoS1) and correlated ACK/Completion messages on `devices/<id>/cmd/resp` (`status: ack|done|error|busy`, structured warnings/errors, shared message IDs, duplicate suppression). Firmware integrates the MotorCommandProcessor pipeline, BUSY rejection, command result mapping, and config commands (`MQTT:GET/SET_CONFIG`) using Preferences. CLI/TUI MQTT mode publishes single-action requests, tracks ACK latency, surfaces BUSY/ERROR states, and mirrors serial logs without blocking telemetry. Acceptance: unit/native tests cover JSON validation, duplicate guardrails, BUSY overlap handling, STATUS telemetry parity, config reboot persistence, and CLI command queue behaviour; bench checklist added for MOVE/BUSY/config flows. Depends on: 10. `M`

Items below are implemented in [mirror-matrix-control-ui](https://github.com/szerintedmi/mirror-matrix-control-ui). Kept here for refernce but for up-to-date info see that repo.
